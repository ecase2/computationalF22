...
@with_kw struct Primitives
    ...
    trans_matrix = [0.977 0.023; 0.074 0.926]
end
...
function Bellman(prim::Primitives,res::Results)
    ...
    @unpack k_grid, β, δ, α, nk, nz, z_grid, trans_matrix  = prim #unpack model primitives
        ...
        for k_index = 1:nk
            ...
            for kp_index in choice_lower:nk #loop over possible selections of k', exploiting monotonicity of policy function
                ...
                if c>0 #check for positivity
                    val = log(c) + β*transpose(trans_matrix[z_index, :])*val_func[kp_index, :]   #compute value
